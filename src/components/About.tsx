...
// Dynamically import all worker images (sorted numerically)
const importImages = import.meta.glob('/src/assets/gallery/workers/*.jpg', {
  eager: true,
  query: '?url',         // new way to get file URLs
  import: 'default',     // import the default export (URL string)
});

function extractNumberFromPath(path: string) {
  const m = path.match(/worker-(\d+)\.jpg$/i);
  return m ? Number(m[1]) : 0;
}

const HOUSEKEEPING_TEAM_IMAGES = Object.entries(importImages)
  .map(([path, url]) => ({ path, url: url as string }))
  .sort((a, b) => extractNumberFromPath(a.path) - extractNumberFromPath(b.path))
  .map(({ url }) => ({
    src: url,
    alt: url.split('/').pop()?.replace(/\.\w+$/, '') ?? 'Worker photo',
  }));
